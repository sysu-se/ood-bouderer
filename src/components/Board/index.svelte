<script>
	...
	import { gameSession, invalidCells } from '@sudoku/stores/gameSession';
			...
			{#each $gameSession as row, y}
				{#each row as value, x}
					<Cell {value}
					      ...
					      sameNumber={$settings.highlightSame && value && !isSelected($cursor, x, y) && getValueAtCursor($gameSession, $cursor) === value}
